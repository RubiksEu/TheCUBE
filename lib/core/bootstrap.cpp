                                ...
                                coap_packet_t *response)
{
    ...
    case COAP_PUT:
    {
        ...
        else
        {
            ...
            {
                #ifdef DEBUG_LOG
                Log.notice("[BOOTSRAP]        if (message->payload_len == 0 || message->payload == 0)  COAP_400");
                #endif
            ...
            else
            {
                ...
                for (i = 0; i < size; i++)
                {
                    ...
                    {
#ifdef DEBUG_LOG
                        Log.notice("[BOOTSRAP]        459      else COAP_400");
#endif
                        result = COAP_400_BAD_REQUEST;
                    }
                }
    ...
    case COAP_DELETE:
    {
        ...
        {
            #ifdef DEBUG_LOG
            Log.notice("[BOOTSRAP]       if (LWM2M_URI_IS_SET_RESOURCE(uriP))     else COAP_400");
            #endif
            result = COAP_400_BAD_REQUEST;
        }
        else
    ...
    default:
#ifdef DEBUG_LOG
        Log.notice("[BOOTSRAP]      498  default   else COAP_400");
#endif
        result = COAP_400_BAD_REQUEST;
        break;
    }
                                ...
                                coap_packet_t *response)
{
    ...
    {
        Log.notice("[BOOTSRAP]      if (message->code != COAP_POST)   else COAP_400");
        return COAP_400_BAD_REQUEST;
    }
    if (message->uri_query == NULL)
    {
        Log.notice("[BOOTSRAP]      if (message->uri_query == NULL)    else COAP_400");
        return COAP_400_BAD_REQUEST;
    }
    if (message->payload != NULL)
    {
        Log.notice("[BOOTSRAP]      if (message->payload != NULL)  else COAP_400");
        return COAP_400_BAD_REQUEST;
    }

    if (lwm2m_strncmp((char *)message->uri_query->data, QUERY_NAME, QUERY_NAME_LEN) != 0)
    {
        Log.notice("[BOOTSRAP]    if (lwm2m_strncmp((char *  else COAP_400");
        return COAP_400_BAD_REQUEST;
    }

    if (message->uri_query->len == QUERY_NAME_LEN)
    {
        Log.notice("[BOOTSRAP]    if (message->uri_que  else COAP_400");
        return COAP_400_BAD_REQUEST;
    }
    if (message->uri_query->next != NULL)
    {
        Log.notice("[BOOTSRAP]     if (message->uri_query->ne  else COAP_400");
        return COAP_400_BAD_REQUEST;
    }
                          ...
                          size_t length)
{
    ...
    {
        Log.notice("[BOOTSRAP]      if (uriP == NULL || buffer == NULL  || length == 0)  else COAP_400");
        return COAP_400_BAD_REQUEST;
    }
