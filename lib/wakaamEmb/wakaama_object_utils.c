...
static lwm2m_object_res_item_t* prv_find_ressource(lwm2m_object_meta_information_t* metaP, uint16_t id) {
    ...
    lwm2m_log("prv_find_ressource");   
    lwm2m_log_int(id);   
    if (!(metaP->ressources_len))
        return NULL;
    
    for(unsigned i=0;i<metaP->ressources_len;++i)
    {
        lwm2m_object_res_item_t* resP = &(metaP->ressources[i]);
        lwm2m_log_int(resP->ressource_id);   
        if (resP->ressource_id == id) {
        return &metaP->ressources[i];
        }
    }
                        ...
                        lwm2m_object_t * objectP)
{
    ...
    lwm2m_log("metaP found");   
    lwm2m_log_int(metaP->instance_object_size);
    lwm2m_log_int(metaP->ressources_len);
    lwm2m_log_int(metaP->ressources[0].ressource_id);
    lwm2m_log_int(metaP->ressources[0].type_and_access);
   lwm2m_log_int(metaP->ressources[0].struct_member_offset);
    ...
    for (int i = 0 ; i < *numDataP ; i++)
    {
        ...
        lwm2m_log("res_id");   
        lwm2m_log_int(res_id);  
        lwm2m_log_int(i);   
        
        lwm2m_object_res_item_t* resP = prv_find_ressource(metaP, res_id);
        if (!resP)
            return COAP_404_NOT_FOUND;
        lwm2m_log("resP found");   
        
        lwm2m_log_int(resP->type_and_access);   
        lwm2m_log_int(resP->struct_member_offset);   
        lwm2m_log_int(resP->ressource_id);   
        
        if (resP->type_and_access & O_RES_R ){
            lwm2m_log("resP->type_and_access & O_RES_R ");   
        }
        if (resP->struct_member_offset==0 ){
           lwm2m_log("resP->struct_member_offset==0");   
        }
        if (!(resP->type_and_access & O_RES_R) || resP->struct_member_offset==0)
            return COAP_405_METHOD_NOT_ALLOWED;
        lwm2m_log("!COAP_405_METHOD_NOT_ALLOWED");   
        lwm2m_object_assign_single_value(dataP, resP, instanceP);
    }
    lwm2m_log("Exit prv_read");   
    return COAP_205_CONTENT;
}
