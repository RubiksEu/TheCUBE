                               ...
                               uint16_t id)
{
    ...
    // lwm2m_log("do lwm2m_list_find");   
    // lwm2m_log_int(id);   
    while (NULL != head && head->id < id)
    {
        // lwm2m_log("do lwm2m_list_find loop");   
        // lwm2m_log_int(head->id);   
        head = head->next;
    }
    // lwm2m_log("do lwm2m_list_find end loop");   
    // if (NULL != head)
    // {
    // lwm2m_log_int(head->id);   
    // }
    // else
    // {
    //     lwm2m_log("head is null");   
    // }
    
    if (NULL != head && head->id == id) return head;
    // lwm2m_log("do lwm2m_list_find not found");   
    return NULL;
}
