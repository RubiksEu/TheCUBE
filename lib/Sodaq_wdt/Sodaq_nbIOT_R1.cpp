...
void Sodaq_nbIOT::close_socket_s(int socket)
{
    char respone[BUFFER_SIZE];
    char nsocl[BUFFER_SIZE] = {0};
    sprintf(nsocl, "AT+NSOCL=%d", socket);
    println(nsocl);
    delay(100);
    debugPrintLn(readResponse());
//     serial_read(fd, respone, 512, 10000);
}

int Sodaq_nbIOT::create_socket_s(const char *portStr)
{

    purgeAllResponsesRead();

    debugPrintLn("Create socket");
    int socket = -1;
    char open_socket[BUFFER_SIZE] = {0};
    sprintf(open_socket, "AT+NSOCR=\"DGRAM\",17,%s,1", portStr);
    debugPrintLn("\n\nClose socket 0");
    close_socket_s(0);
    debugPrintLn("Close socket 1");
    close_socket_s(1);

    debugPrintLn(open_socket);
    println(open_socket);
    delay(500);
...
/* Send N bytes of BUF on socket FD to peer at address ADDR (which is
   ...
   __THROW.  */
ssize_t Sodaq_nbIOT::sendto_s(const void *__buf, size_t __n)
{
    int socket = 0;

    char nsost[BUFFER_SIZE] = {0};
    char hex[BUFFER_SIZE] = {0};
#ifdef DEBUG_LOG
    debugPrintLn("\n[Sending] - sendto_s");
    debugPrint("[Sending] buffer: ");
    Log.notice("%s \n", __buf);
#endif
    string_to_hex((const char *)__buf, __n, hex);
    // socket, ip address, port, decoded message length, hex encoded message
#ifdef DEBUG_LOG
    debugPrintLn("[Sending] - prepare AT command");
#endif
    sprintf(nsost, "AT+NSOST=%d,\"%s\",%d,%d,\"%s\"", socket, getServerIP(), getPort(), (int)(strlen(hex) / 2), hex);
#ifdef DEBUG_LOG
    debugPrint("[Sending] ");
    debugPrintLn(nsost);
#endif
...
int Sodaq_nbIOT::recvfrom_s(char *__restrict __buf, size_t __n)
{
#ifdef DEBUG_LOG
    debugPrintLn("recvfrom_s");
#endif
    println("AT+NSORF=0,512");
    char nsorf_message[BUFFER_SIZE] = {0};
    int length;
    length = readBytes((uint8_t *)nsorf_message, __n, 70); 
#ifdef DEBUG_LOG
    debugPrintLn(nsorf_message);
#endif
    purgeAllResponsesRead();

    length = parse_nsorf_response(nsorf_message, __buf);
    return length;
}

int Sodaq_nbIOT::parse_nsocr_response(const char *response)
{
    int socket = -1;
    if (isdigit(response[2]))
    {
        socket = atoi(response);
    }
    return socket;
}

/*
 * Make sure, that message buffer is big enough to store decoded hex
 */
void Sodaq_nbIOT::hex_to_string(char *message, const char *hex, int length)
{
    unsigned int u;
    int i = 0;

    while (i < length && sscanf(hex, "%2x", &u) == 1)
    {
        message[i++] = (char)u;
        hex += 2;
    }
...
int Sodaq_nbIOT::parse_nsorf_response(const char *response, char *message)
{
    char hex_string[BUFFER_SIZE * 2] = {0};
    bool found_message = false;
    int i = 0;
    char parsed[6][BUFFER_SIZE] = {0};

    /*while (!found_message && i++ < length) {
        sscanf(&response[i], "%d,%s,%d,%d,%s,%d", &socket, address, &port, &message_length, hex_string, &remaining_length);
        if (port != 0) {
            found_message = true;
        }
    }*/

    // find a valid message in response
    char *pt;
    pt = strtok((char *)response, "\n");
    while (pt != NULL && !found_message)
    {
        if (isdigit(pt[0]))
        {
            found_message = true;
    ...
    // parse comma-separated response string
    i = 0;
    pt = strtok(pt, ",");
    while (pt != NULL)
    {
    ...
    if (atoi(parsed[3]) != 0)
    {
        message_length = atoi(parsed[3]);
        pt = strtok(parsed[4], "\"");
        strcpy(parsed[4],pt);

        hex_to_string(message, parsed[4], message_length);
    }
    return message_length;
}

/*
 * Make sure, that hex buffer is big enough to store encoded message
 */
void Sodaq_nbIOT::string_to_hex(const char *message, size_t length, char *hex)
{
    int i, j;
    for (i = 0, j = 0; i < length; i++, j += 2)
    {
        sprintf(hex + j, "%02X", (__uint8_t)message[i]);
    }
    hex[j] = '\0';
}
...
{
    debugPrintLn("[setRadioActive] processing...");
    if (on)
    {
        debugPrintLn("[setRadioActive] trying radio ON");
    }
    else
    {
        debugPrintLn("[setRadioActive] trying radio OFF");
    };
    print("AT+CFUN=");
    println(on ? "1" : "0");
                                        ...
                                        size_t *outSize, uint32_t timeout)
{
    ...
    do
    {
        ...
        if (count > 0)
        {
            ...
            if (!parserMethod && strstr(buffer, "CGATT") != NULL)
            {
...
{
    debugPrintLn("[setApn]: inside setApn...");
...
{
    debugPrintLn("[setCdp]: inside setCdp...");
    print("AT+NCDP=\"");
    print(cdp);
    println("\"");
...
void Sodaq_nbIOT::purgeAllResponsesRead()
{
    ...
    while ((readResponse(0, 70) != ResponseTimeout) && !is_timedout(start, 180))
    {
    }
}

void Sodaq_nbIOT::readBuffer()
{
    uint32_t start = millis();
    // make sure all the responses within the timeout have been read
    while ((readResponse(0, 50) != ResponseTimeout) && !is_timedout(start, 100))
    {
    }
}

uint8_t loading[] = {6, 10, 18, 20, 16, 12, 6, 10, 6};

uint8_t load_iter = 0;
uint8_t start = 39;

/*
* Show animation when cube tries to connect to network
*/
void load(Adafruit_SSD1306 display, uint8_t delay_count)
{
    for (uint8_t j = 0; j < delay_count; j++)
    {
        if (load_iter >= 8)
        {
            load_iter = 0;
        ...
        for (int16_t i = 0; i < 8; i++, load_iter++)
        {
            display.fillRect(start, display.height() / 2 - loading[load_iter] / 2 - 3, 3, loading[load_iter], INVERSE);
            display.display();
            start += 6;
        }
    }
...
//------------------------------------------------------------------------------------------------------
bool Sodaq_nbIOT::connect(Adafruit_SSD1306 display, ConfigJson *commands, uint8_t count)
{
    load_iter = 0;
    start = 39;
    if (!on())
    {
        debugPrintLn("[!on]: seems the UBLOX is off...");
        return false;
    }
    purgeAllResponsesRead();
    debugPrint("[connect]: start loop");
    for (uint8_t i = 0; i < count; i++)
    {
        //retries
        for (uint8_t j = 0; j < commands[i].retries; j++)
        {
            debugPrint("[command]: ");
            debugPrintLn(commands[i].command);

            println(commands[i].command);
            ResponseTypes response = readResponse();
            if (commands[i].purge)
            {
                purgeAllResponsesRead();
            }
            load(display, commands[i].delay_time);
...
bool Sodaq_nbIOT::connect(Adafruit_SSD1306 display, const char *apn, const char *cdp, const char *forceOperator)
{

    debugPrintLn("[connect]: starting with connect procedure...");
    purgeAllResponsesRead();
    if (!on())
    {
        debugPrintLn("[!on]: seems the UBLOX is off...");
    ...
    debugPrintLn("[connect]: trying to set NCONFIG parameters...");
    setNconfigParam_custom("\"AUTOCONNECT\"", "\"FALSE\"");
    delay(100);

    debugPrintLn("[connect]: trying to set NCONFIG parameters...");
    setNconfigParam_custom("\"CR_0354_0338_SCRAMBLING\"", "\"TRUE\"");
    delay(100);
    setNconfigParam_custom("\"CR_0859_SI_AVOID\"", "\"FALSE\"");
    delay(100);
    debugPrintLn("[connect]: NCONFIG parameters done...");
    ...
    debugPrintLn("[connect] Set indications");
    if (!setIndicationsActive(false))
    {
        return false;
    }
    debugPrintLn("[connect] Set NBAND");
    print("AT+NBAND=");
    println("20");
    debugPrintLn(readResponse());
    //
    ...
    if (!attachGprs(90000))
    {
        debugPrintLn("[!attachGprs]: problems with Gprs...");
        return false;
    }
...
{
    debugPrintLn("[reboot]: rebooting AT+NRB");
    println("AT+NRB");
...
bool Sodaq_nbIOT::checkAndApplyNconfig()
{
    ...
    debugPrintLn("[checkAndApplyNconfig]: processing...");
...
//
//-----------------------------------------------------------------------------
bool Sodaq_nbIOT::setNconfigParam_custom(const char *param, const char *value)
{
    print("AT+NCONFIG=");
    print(param);
    print(",");
...
bool Sodaq_nbIOT::attachGprs(uint32_t timeout)
{
    ...
    while (!is_timedout(start, timeout))
    {
        if (isConnected())
        {
            debugPrintLn("Is Connected return true");
            return true;
        }
        //println("AT+CGATT=1");
        ...
        if (delay_count < 10000)
        {
            ...
            debugPrintLn("Error return true");
    return false;
}
//================================================================================
...
void Sodaq_nbIOT::init(const char *id, const char *ip, const char *port)
{
    debugPrintLn("Try to init");
    // coapPort = (char *)calloc(strlen(port), sizeof(char));
   
    // cubeId = (char *)calloc(strlen(id), sizeof(char));
    // serverIp = (char *)calloc(strlen(ip), sizeof(char));

}

void Sodaq_nbIOT::setName(const char *id)
{
    strcpy(cubeId, id);
}

const char *Sodaq_nbIOT::getName()
{
    return cubeId;
}

void Sodaq_nbIOT::setServerIP(const char *ip)
{
    strcpy(serverIp, ip);
}

const char *Sodaq_nbIOT::getServerIP()
{
    return serverIp;
}

void Sodaq_nbIOT::setPort(int port)
{
     coapPort = port;
}

int Sodaq_nbIOT::getPort()
{
    return coapPort;
}
