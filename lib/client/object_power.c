...
{
    power_instance_t * targetP;
    int i;

    targetP = (power_instance_t *)lwm2m_list_find(objectP->instanceList, instanceId);
                        ...
                        lwm2m_object_t * objectP)
{
    ...
    if (*numDataP == 0)
    {
        ...
        *numDataP = 8;
        (*dataArrayP)[0].id = MIN_POW_MEASURED_VALUE;
        (*dataArrayP)[1].id = MAX_POW_MEASURED_VALUE;
        (*dataArrayP)[2].id = MIN_POW_RANGE_VALUE;
        (*dataArrayP)[3].id = MAX_POW_RANGE_VALUE;
        (*dataArrayP)[4].id = SENSOR_POW_VALUE;
        (*dataArrayP)[5].id = SENSOR_POW_UNITS;
        (*dataArrayP)[6].id = APPLICATION_POW_TYPE;
        (*dataArrayP)[7].id = CURRENT_POW_CALIBRATION;
    }
    for (i = 0 ; i < *numDataP ; i++)
    {
        switch ((*dataArrayP)[i].id)
        {
            case MIN_POW_MEASURED_VALUE:
                lwm2m_data_encode_float((double)targetP->min_measured_value, *dataArrayP + i);
                break;
            case MAX_POW_MEASURED_VALUE:
                lwm2m_data_encode_float((double)targetP->max_measured_value, *dataArrayP + i);
                break;
            case MIN_POW_RANGE_VALUE:
                lwm2m_data_encode_float((double)targetP->min_range_value, *dataArrayP + i);
                break;
            case MAX_POW_RANGE_VALUE:
...
lwm2m_object_t * init_power_object()
{
    ...
    if (NULL != testObj)
    {
        ...
        testObj->objID = POWER_OBJECT_ID;
        for (i=0 ; i < 1 ; i++)
        {
            targetP = (power_instance_t *)lwm2m_malloc(sizeof(power_instance_t));
            if (NULL == targetP) return NULL;
            memset(targetP, 0, sizeof(power_instance_t));
            targetP->shortID = LWM2M_POWER_SHORT_OBJECT_ID;
