...
{
    air_instance_t * targetP;
    int i;

    targetP = (air_instance_t *)lwm2m_list_find(objectP->instanceList, instanceId);
                        ...
                        lwm2m_object_t * objectP)
{
    ...
    if (*numDataP == 0)
    {
        ...
        *numDataP = 7;
        (*dataArrayP)[0].id = SENSOR_A_CURRENT_VALUE;
        (*dataArrayP)[1].id = MAX_A_MEASURED_VALUE;
        (*dataArrayP)[2].id = MAX_A_RANGE_VALUE;
        (*dataArrayP)[3].id = MIN_A_MEASURED_VALUE;
        (*dataArrayP)[4].id = MIN_A_RANGE_VALUE;
        (*dataArrayP)[5].id = APPLICATION_A_TYPE;
        (*dataArrayP)[6].id = SENSOR_A_TYPE;
    }
    for (i = 0 ; i < *numDataP ; i++)
    {
        switch ((*dataArrayP)[i].id)
        {
            case MIN_A_MEASURED_VALUE:
                lwm2m_data_encode_float((double)targetP->min_measured_value, *dataArrayP + i);
                break;
            case MAX_A_MEASURED_VALUE:
                lwm2m_data_encode_float((double)targetP->max_measured_value, *dataArrayP + i);
                break;
            case MIN_A_RANGE_VALUE:
                lwm2m_data_encode_float((double)targetP->min_range_value, *dataArrayP + i);
                break;
            case MAX_A_RANGE_VALUE:
...
lwm2m_object_t * init_air_object()
{
    ...
    if (NULL != testObj)
    {
        ...
        testObj->objID = AIR_OBJECT_ID;
        for (i=0 ; i < 1 ; i++)
        {
            targetP = (air_instance_t *)lwm2m_malloc(sizeof(air_instance_t));
            if (NULL == targetP) return NULL;
            memset(targetP, 0, sizeof(air_instance_t));
            targetP->shortID = LWM2M_AIR_SHORT_OBJECT_ID;
