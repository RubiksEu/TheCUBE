...
{
    collision_instance_t * targetP;
    int i;

    targetP = (collision_instance_t *)lwm2m_list_find(objectP->instanceList, instanceId);
    ...
    for (i = 0 ; i < numData ; i++)
    {
        switch (dataArray[i].id)
        {
            ...
            case APPLICATION_C_TYPE:
                ...
                break;
            case OFF_C_TIME:
                if (1 != lwm2m_data_decode_int(dataArray + i, &(targetP->off_time)))
                {
                    return COAP_400_BAD_REQUEST;
                }
                break;
                        ...
                        lwm2m_object_t * objectP)
{
    ...
    if (*numDataP == 0)
    {
        ...
        *numDataP = 5;
        (*dataArrayP)[0].id = APPLICATION_C_TYPE;
        (*dataArrayP)[1].id = DIGITAL_C_INPUT_STATE;
        (*dataArrayP)[2].id = DIGITAL_C_INPUT_STATE_2;
        (*dataArrayP)[3].id = OFF_C_TIME;
        (*dataArrayP)[4].id = ON_C_TIME;
    }
    for (i = 0 ; i < *numDataP ; i++)
    {
        switch ((*dataArrayP)[i].id)
        {
            case DIGITAL_C_INPUT_STATE:
                ...
                break;
            case DIGITAL_C_INPUT_STATE_2:
                lwm2m_data_encode_int(targetP->digital_input_state, *dataArrayP + i);
                break;
            case OFF_C_TIME:
                lwm2m_data_encode_int(targetP->off_time, *dataArrayP + i);
                break;
            case ON_C_TIME:
...
lwm2m_object_t * init_collision_object()
{
    ...
    if (NULL != testObj)
    {
        ...
        testObj->objID = COLLISION_OBJECT_ID;
        for (i=0 ; i < 1 ; i++)
        {
            targetP = (collision_instance_t *)lwm2m_malloc(sizeof(collision_instance_t));
            if (NULL == targetP) return NULL;
            memset(targetP, 0, sizeof(collision_instance_t));
