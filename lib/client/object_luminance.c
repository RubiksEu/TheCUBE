...
{
    luminance_instance_t * targetP;
    int i;

    targetP = (luminance_instance_t *)lwm2m_list_find(objectP->instanceList, instanceId);
                        ...
                        lwm2m_object_t * objectP)
{
    ...
    if (*numDataP == 0)
    {
        ...
        *numDataP = 6;
        (*dataArrayP)[0].id = MIN_LUM_MEASURED_VALUE;
        (*dataArrayP)[1].id = MAX_LUM_MEASURED_VALUE;
        (*dataArrayP)[2].id = MIN_LUM_RANGE_VALUE;
        (*dataArrayP)[3].id = MAX_LUM_RANGE_VALUE;
        (*dataArrayP)[4].id = SENSOR_LUM_VALUE;
        (*dataArrayP)[5].id = SENSOR_LUM_UNITS;
    }
...
lwm2m_object_t * init_luminance_object()
{
    ...
    if (NULL != testObj)
    {
        ...
        testObj->objID = LUMINANCE_OBJECT_ID;
        for (i=0 ; i < 1 ; i++)
        {
            targetP = (luminance_instance_t *)lwm2m_malloc(sizeof(luminance_instance_t));
            if (NULL == targetP) return NULL;
            memset(targetP, 0, sizeof(luminance_instance_t));
            targetP->shortID = LWM2M_LUMINANCE_SHORT_OBJECT_ID;
