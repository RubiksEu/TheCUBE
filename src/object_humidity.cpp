...
static uint8_t prv_humidity_read(uint16_t instanceId, int *numDataP, lwm2m_data_t **dataArrayP, lwm2m_object_t *objectP)
{
    ...
    int i;
#ifdef DEBUG
    Serial.println("{OBJECT_TEMPERATURE}{prv_humidity_read}");
#endif
    targetP = (humidity_instance_t *)lwm2m_list_find(objectP->instanceList, instanceId);
    if (NULL == targetP)
        return COAP_404_NOT_FOUND;
                        ...
                        lwm2m_object_t *objectP)
{
    ...
    case RESET_H_MEASURED_VALUE:
        if (execHumidity)
        {
            digitalWrite(THRESHOLD_LED_2_PIN, HIGH);
        }
        else
        {
            digitalWrite(THRESHOLD_LED_2_PIN, LOW);
        }
        execHumidity = !execHumidity;
        return COAP_204_CHANGED;
    default:
        return COAP_404_NOT_FOUND;
    }
...
{
#ifdef DEBUG
    Serial.println("init_humidity_object");
#endif
    ...
    if (NULL != humidityObj)
    {
        ...
        humidityObj->objID = LWM2M_HUMIDITY_OBJECT_ID; 
        for (i = 0; i < 1; i++)
        {
